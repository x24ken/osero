...
//勝利判定を行いたいが、リロードがあるたびに、勝利判定を行う奴があったはず
// useEffect学んだ後に再挑戦

const Othello = () => {
  const [othello, setOthello] = useState([
  ...
  const computerClick = () => {
    const possibleOthelloArray = checkReturnOthello(othello, !blackIsNext);
    if (possibleOthelloArray === 0) {
      setFinish(true);
      return;
    }
    const random = Math.floor(Math.random() * possibleOthelloArray.length);
    const changeOthelloArray = possibleOthelloArray[random];

    const newChangeOthello = newOthello(
      othello,
      changeOthelloArray,
      !blackIsNext ? "black" : "white"
    );
    setOthello(newChangeOthello);
    setBlackIsNext(blackIsNext);
  };

  const humanClick = (yIndex, xIndex) => {
    if (othello[yIndex][xIndex]) {
      return;
    }
    const changeArray = checkOthelloAround(
      othello,
      yIndex,
      xIndex,
      blackIsNext
    ...
    const newOthello = [
      ...
      changeArray.forEach((change) => {
        othello[change[0]][change[1]] = blackIsNext ? "black" : "white";
      }),
    ];
  ...
  const status = blackIsNext ? "黒の番です" : "白の番です。";

  let blackCount = 0;
  let whiteCount = 0;
  for (let y = 0; y < 8; y++) {
    for (let x = 0; x < 8; x++) {
      if (othello[y][x] === "black") {
        blackCount += 1;
      }
      if (othello[y][x] === "white") {
        whiteCount += 1;
      }
    }
  }
  const finishMessage = finish && "おめでとう！";
  ...
  return (
    <div className="game">
      <div className="game-board">
        <Board
          othello={othello}
          onClick={(yIndex, xIndex) => humanClick(yIndex, xIndex)}
        />
      </div>
      <div className="game-info">
        <div>{status}</div>
        <div>{`黒: ${blackCount} vs 白: ${whiteCount} `}</div>
...
//ヘルパー関数・・・
const checkReturnOthello = (othello, blackIsNext) => {
